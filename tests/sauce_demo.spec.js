...
try {
  ...
  console.log("GitHub secrets used for Credentials...");
}

//This describe block allows you to run the entire test suite at once
test.describe("Run the entire Test Suite", () => {
  ...
  test.describe("Group of Login Functionality Tests", () => {
    //Test the login functionality using standard crede
    test("Standard Login Test", async ({ page }) => {
      const navigator = new webpageNavigators(page);
      const userName =
        credentials.standard_user || process.env.STANDARD_USERNAME;
      const passWord =
  ...
  test.describe("Tests that pertain to the purchase flow", () => {
    ...
    //Test the entire purchasing process flow of three arbitrary items
  ...
  test.describe("Tests that pertain to the shopping cart functionalities and purchasing", () => {
    ...
    test("Test removal of an item from the shopping cart,", async ({
      page,
    }) => {
      const navigator = new webpageNavigators(page);
